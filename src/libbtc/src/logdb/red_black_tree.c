...
/*  key is deleted.  info_destroy_callback is similiar to DestFunc except it */
...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
			      void (*destroy_key_callback) (void*),
  ...
  newTree=(rb_red_blk_tree*) safe_malloc(sizeof(rb_red_blk_tree));
  newTree->Compare=  CompFunc;
  newTree->destroy_key_callback= destroy_key_callback;
  ...
  temp=newTree->nil= (rb_red_blk_node*) safe_malloc(sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= (rb_red_blk_node*) safe_malloc(sizeof(rb_red_blk_node));
...
/*              postorder using the destroy_key_callback and  */
/*              info_destroy_callback functions.                        */
/**/
/*    Modifies Input: tree, x */
/**/
...
void TreeDestHelper(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != nil) {
    ...
    tree->destroy_key_callback(x->key);
    tree->info_destroy_callback(x->info);
    free(x);
  }
}
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  if (y != z) { /* y should not be nil in this case */
    ...
    tree->destroy_key_callback(z->key);
    tree->info_destroy_callback(z->info);
    y->left=z->left;
    y->right=z->right;
    y->parent=z->parent;
    y->red=z->red;
    ...
    tree->destroy_key_callback(y->key);
    tree->info_destroy_callback(y->info);
    if (!(y->red)) RBDeleteFixUp(tree,x);
    free(y);
  }
}
